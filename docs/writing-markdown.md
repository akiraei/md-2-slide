...
## 자료
필요한 경우 실습용 HTML/CSS 코드와 파일명 예시
~~~
...
- `발표 메모` should contain detailed explanation, examples, analogies, and
  presenter flow.
- `자료` should contain practice assets, not normal visible slide copy.
...
Keep code examples short enough to read at slide size. If the slide only needs
a short excerpt, split it across slides or move details to `발표 메모`. If the
code is a complete reusable practice file, put it in `자료`.
...
## Practice Asset Usage

Use `자료` when a slide needs a full HTML/CSS example that students should see
as a rendered result and also reuse as source code.

This section is for practice assets attached to a slide. It is not normal slide
body copy and should not be mixed into `화면 내용`.
...
<body>
  <h1>광주 동네 카페</h1>
  <p>조용한 골목에 있는 따뜻한 분위기의 카페입니다.</p>
</body>
</html>
```
~~~

For PPTX generation, `자료` should be handled as a two-part learning asset:

- The rendered preview of the HTML/CSS belongs on the original slide.
- The source code belongs on a generated material slide immediately after the
  original slide.
- The material slide should make the code easy to copy.
- The `파일명 예시` value should be used as the file label when present.
- Long code may be split across multiple material slides instead of shrinking
  the text until it is unreadable.

Use `화면 내용` code blocks for short code examples that should be visible on the
main slide. Use `자료` for longer complete files that students should run or
copy during practice.
...
- Did long reusable practice code move to `자료`?
...
- If `자료` is used, is there a clear file label and a complete code block?
...
7. Put reusable practice files in `자료`.
8. Run validation before building the deck.
